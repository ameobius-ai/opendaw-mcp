#!/usr/bin/env python3
"""Post-production mastering — PRO version.

v7 upgrades over v6:
1. Pre-emphasis/de-emphasis around tape sat (analog HF smoothing)
2. Side-channel boost on air (10-12kHz stereo widening)
3. Tube saturation on bass band (even harmonics on lows)
4. Program-dependent release on limiter (faster on transients, slower on sustains)

Usage:
  venv/bin/python post_master_pro.py <input.wav> [output_name]
"""
import sys
import os
import numpy as np
...
def main():
    ...
    # Sub (<200Hz): psychoacoustic bass + transient enhancer + parallel comp + mono
    print("  sub: psychoacoustic bass + transient enhancer (NEW v8) + par.comp + mono")
    bands[0] = psychoacoustic_bass(bands[0], sr, freq=60, amount=0.15, mix=0.2)
    bands[0] = transient_enhancer(bands[0], sr, freq_low=50, freq_high=200, amount=0.3, attack_ms=5.0, mix=0.4)
    ...
    # Bass (200-500Hz): mud cut + tube sat
    print("  bass: mud cut -2dB @ 250 + tube sat")
    bands[1] = bell_eq(bands[1], sr, 250, -2.0, q=1.2)
    bands[1] = tube_saturation(bands[1], drive=0.15, mix=0.3)

    # Lowmid (500-2k): warmth fill + DYNAMIC EQ (NEW v8) + pre-emphasis tape sat
    print("  lowmid: +3dB @ 700 + dynamic EQ -2dB @ 300 (NEW v8) + pre-emph tape")
    ...
    bands[2] = tape_sat_pre_emphasis(bands[2], sr, drive=0.1, mix=0.2, emph_freq=4000, emph_gain=3.0)

    # Presence (2k-5k): boost + tube sat + parallel comp
    print("  presence: +5dB @ 3k + +4dB @ 4k + +3dB @ 5k + tube + par.comp")
    ...
    pres_comp = compressor(bands[3], threshold=0.3, ratio=3.0, attack=0.005, release=0.15, sr=sr, mix=1.0)
    bands[3] = bands[3] * 0.6 + pres_comp * 0.4

    # Upper-mid (5k-8k): vocal clarity
    print("  uppermid: +2dB @ 6k (vocal clarity)")
    bands[4] = bell_eq(bands[4], sr, 6000, 2.0, q=1.0)

    # Air (>8k): boost + EVEN harmonic exciter (NEW v8) + widen
    print("  air: high-shelf +4dB + EVEN exciter (NEW v8) + bell +3dB @ 12k + widen")
    bands[5] = high_shelf(bands[5], sr, 8000, 4.0)
    bands[5] = harmonic_exciter_even(bands[5], sr, freq=8000, amount=0.25, mix=0.4)
    ...
    # ═══ Step 5: M/S EQ — side HPF + side air boost (NEW v7) ═══
    print("\n=== Step 5: M/S EQ (side HPF 200Hz + side boost +3dB @ 10k, NEW v7) ===")
    data = ms_eq(data, sr, side_hp_freq=200, side_boost_freq=10000, side_boost_gain=3.0)
    ...
    # ═══ Step 7: Soft knee + oversampled true peak limiter (NEW v8) ═══
    print("\n=== Step 7: Soft knee + oversampled limiter (NEW v8) ===")
    # First pass: soft knee for transparent gain reduction
    data = soft_knee_limiter_prog(data, ceiling=0.86, knee_db=3.0, sr=sr,
                                  lookahead_ms=3.0, release_fast_ms=50.0, release_slow_ms=150.0)
    # Second pass: oversampled hard limit catches intersample peaks
    # ceiling 0.86 = -1.3 dBFS, true peak after OS will be ~-1.0 dBTP
    data = oversampled_limiter(data, ceiling=0.86, sr=sr, oversample=4)
    ...
    if abs(correction) > 0.1:
        data = data * (10 ** (correction / 20))
        # Oversampled limit again if correction raised peaks
        data = oversampled_limiter(data, ceiling=0.86, sr=sr, oversample=4)
    lufs_final = meter.integrated_loudness(data.T)
