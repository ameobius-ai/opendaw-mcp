#!/usr/bin/env python3
"""E2E test for werkstatt_vinyl.js — vinyl record simulator."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_vinyl.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            expected_labels = {"age", "dust", "wear", "wow", "flutter", "noise", "mix", "output"}
            if r3.get("params"):
            ...
            if r3.get("params"):
                d = [p for p in r3["params"] if p["label"] == "dust"]
                if d and abs(d[0]["def"] - 0.5) < 0.01:
                    passed += 1
                else:
                    print(f"  ❌ dust default: {d}")
                    failed += 1
            else:
                failed += 1
            ...
            if r8.get("newVal") is not None and abs(r8["newVal"] - 0.3) < 0.01:
                passed += 1
            else:
                print(f"  ❌ set wow: {r8}")
                failed += 1
            ...
            await bridge.stop()
            print(f"werkstatt_vinyl E2E: {passed}/{passed + failed}")
