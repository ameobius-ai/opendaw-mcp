#!/usr/bin/env python3
"""E2E test for werkstatt_spectral_gate.js DSP script."""
...
CODE = open(os.path.join(os.path.dirname(SCRIPT_DIR), "scripts", "werkstatt_spectral_gate.js")).read()
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            if r.get("success"): print("  ✅ test 1: audio track created"); passed += 1
            ...
            if r2.get("success"): print(f"  ✅ test 2: Werkstatt added ({r2.get('fxCount')} fx)"); passed += 1
            else: print(f"  ❌ test 2: {r2}"); failed += 1
            ...
            if r3.get("compiled") and r3.get("paramCount") == 10:
                ...
                print(f"  ✅ test 3: compiled, 10 params: {labels}"); passed += 1
            else: print(f"  ❌ test 3: {r3}"); failed += 1

            for test_num, param_name, set_val, check_val in [
                (4, "bands", 16, 16),
                (5, "threshold", 0.1, 0.1),
                (6, "reduction", 0.7, 0.7),
                (7, "tilt", 0.5, 0.5),
                ...
                if r.get("newVal") is not None and abs(r["newVal"] - check_val) < 0.01:
                    print(f"  ✅ test {test_num}: {param_name} {r['oldVal']:.3f} → {r['newVal']:.3f}"); passed += 1
                else: print(f"  ❌ test {test_num}: {r}"); failed += 1
            ...
            if "@werkstatt" in r8.get("header", ""):
                print(f"  ✅ test 8: header OK ({r8['header'][:35]}...)"); passed += 1
            else: print(f"  ❌ test 8: {r8}"); failed += 1

            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_spectral_gate E2E: {passed}/{passed+failed}")
            return failed == 0
