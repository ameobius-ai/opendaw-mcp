#!/usr/bin/env python3
"""E2E test for werkstatt_spectral_compressor.js — STFT spectral compressor."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_spectral_compressor.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            expected_labels = {"threshold", "ratio", "attack", "release", "smoothing", "tilt", "mix", "output"}
            if r3.get("params"):
            ...
            if r3.get("params"):
                s = [p for p in r3["params"] if p["label"] == "smoothing"]
                if s and abs(s[0]["def"] - 0.7) < 0.01:
                    passed += 1
                else:
                    print(f"  x smoothing default: {s}")
                    failed += 1
            else:
                failed += 1
            ...
            if r8.get("newVal") is not None and abs(r8["newVal"] - 0.7) < 0.01:
                ...
                print(f"  x set ratio: {r8}")
                failed += 1

            await bridge.stop()
            print(f"werkstatt_spectral_compressor E2E: {passed}/{passed + failed}")
