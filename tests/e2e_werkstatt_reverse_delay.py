#!/usr/bin/env python3
"""E2E test for werkstatt_reverse_delay.js — reverse delay (The Edge style)."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_reverse_delay.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            # Test 3: compile reverse_delay code
            ...
            expected_labels = {"time", "feedback", "levels", "pan", "fade", "damping", "mix", "output"}
            if r3.get("params"):
            ...
            # Test 5: time default = 0.4
            if r3.get("params"):
                t = [p for p in r3["params"] if p["label"] == "time"]
                if t and abs(t[0]["def"] - 0.4) < 0.01:
                    passed += 1
                else:
                    print(f"  ❌ time default: {t}")
                    failed += 1
            else:
                failed += 1

            # Test 6: set time = 0.75
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 0.75) < 0.01:
                passed += 1
            else:
                print(f"  ❌ set time: {r6}")
                failed += 1

            # Test 7: set feedback = 0.6
            ...
            await bridge.stop()
            print(f"werkstatt_reverse_delay E2E: {passed}/{passed + failed}")
