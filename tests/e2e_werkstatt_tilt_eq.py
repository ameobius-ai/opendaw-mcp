#!/usr/bin/env python3
"""E2E test for werkstatt_tilt_eq.js — single-knob spectral tilt EQ."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_tilt_eq.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            expected_labels = {"tilt", "pivot", "steepness", "mix", "output"}
            if r3.get("params"):
            ...
            if r3.get("params"):
                t = [p for p in r3["params"] if p["label"] == "tilt"]
                if t and abs(t[0]["def"]) < 0.01:
                    passed += 1
                else:
                    print(f"  ❌ tilt default: {t}")
                    failed += 1
            else:
                failed += 1
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 2000) < 1:
                passed += 1
            else:
                print(f"  ❌ set pivot: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"werkstatt_tilt_eq E2E: {passed}/{passed + failed}")
