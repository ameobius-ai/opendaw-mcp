#!/usr/bin/env python3
"""E2E test for spielwerk_prob_gate.js — subtractive probability gate."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "spielwerk_prob_gate.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            expected = {"chance", "variation", "seed", "mode", "min_pitch", "max_pitch", "velocity_boost", "hold"}
            if r3.get("params"):
            ...
            # Test 5: chance default = 0.7
            if r3.get("params"):
                c = [p for p in r3["params"] if p["label"] == "chance"]
                if c and abs(c[0]["def"] - 0.7) < 0.01:
                    passed += 1
                else:
                    print(f"  ❌ chance default: {c}")
                    failed += 1
            else:
                failed += 1

            # Tests 6-8: set params
            for param_name, set_val in [("chance", 0.3), ("mode", 1), ("hold", 0.5)]:
            ...
            await bridge.stop()
            print(f"spielwerk_prob_gate E2E: {passed}/{passed + failed}")
