#!/usr/bin/env python3
"""E2E test for werkstatt_vocoder.js DSP script."""
...
CODE = open(os.path.join(os.path.dirname(SCRIPT_DIR), "scripts", "werkstatt_vocoder.js")).read()
        ...
        async def run():
            ...
            r4 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "bands") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(24);
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 24) < 0.01:
                print(f"  ✅ test 4: bands {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1

            # 5. Set carrier_wave to 1 (square)
            r5 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "carrier_wave") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(1);
                            newVal = pb.value.getValue();
                        }
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - 1) < 0.01:
                print(f"  ✅ test 5: carrier_wave {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1

            # 6. Set carrier_freq to 220
            r6 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "carrier_freq") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(220);
                            newVal = pb.value.getValue();
                        }
                    }
                });
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 220) < 1:
                print(f"  ✅ test 6: carrier_freq {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1

            # 7. Set emphasis to 1.5
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 1.5) < 0.01:
                print(f"  ✅ test 7: emphasis {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_vocoder E2E: {passed}/{passed+failed}")
            return failed == 0
