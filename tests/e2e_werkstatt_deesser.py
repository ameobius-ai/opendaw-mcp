"""E2E test for werkstatt_deesser.js DSP script."""
...
CODE = open("scripts/werkstatt_deesser.js").read()
...
async def main():
    ...
    # 3. Compile de-esser code via ScriptCompiler
    ...
    print("3. Compile de-esser:", f"compiled={r3.get('compiled')}, error={r3.get('error')}, params={r3.get('paramCount')}")
    if r3.get("error"):
        print("   Params detail:", r3.get("params"))
    assert r3.get("compiled"), f"Compile failed: {r3}"
    assert r3.get("paramCount") == 7, f"Expected 7 params, got {r3.get('paramCount')}"
    
    labels = [p["label"] for p in r3["params"]]
    expected = ["freq", "threshold", "ratio", "attack", "release", "mix", "output"]
    for e in expected:
        assert e in labels, f"Missing param: {e}. Got: {labels}"
    print("   All 7 params:", labels)
    
    # 4. Set threshold param
    ...
    print("4. Set threshold:", r4)
    assert abs(r4["newVal"] - 0.65) < 0.01, f"threshold not set: {r4}"
    print(f"   threshold {r4['oldVal']} → {r4['newVal']} ✅")
    ...
    print("5. Set freq:", r5)
    assert abs(r5["newVal"] - 0.8) < 0.01, f"freq not set: {r5}"
    print(f"   freq {r5['oldVal']} → {r5['newVal']} ✅")
