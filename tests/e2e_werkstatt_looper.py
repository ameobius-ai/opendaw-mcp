#!/usr/bin/env python3
"""E2E test for werkstatt_looper.js DSP script."""
...
CODE = open(os.path.join(os.path.dirname(SCRIPT_DIR), "scripts", "werkstatt_looper.js")).read()
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            # 4. Set loop_length to 4 seconds
            r4 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "loop_length") {
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 4) < 0.01:
                print(f"  ✅ test 4: loop_length {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1

            # 5. Set feedback to 0.9
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - 0.9) < 0.01:
                print(f"  ✅ test 5: feedback {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1

            # 6. Set play_mode to 2 (overdub)
            r6 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "play_mode") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(2);
                            newVal = pb.value.getValue();
                        }
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 2) < 0.01:
                print(f"  ✅ test 6: play_mode {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1

            # 7. Set reverse_mode to 1
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "reverse_mode") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(1);
                            newVal = pb.value.getValue();
                        }
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 1) < 0.01:
                print(f"  ✅ test 7: reverse_mode {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_looper E2E: {passed}/{passed+failed}")
            return failed == 0
