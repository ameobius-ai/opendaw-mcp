#!/usr/bin/env python3
"""E2E test for werkstatt_auto_tune.js — pitch detection + correction."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_auto_tune.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            expected_labels = {"key", "scale", "retune", "strength", "detune", "mix", "output"}
            if r3.get("params"):
            ...
            if r3.get("params"):
                s = [p for p in r3["params"] if p["label"] == "retune"]
                if s and abs(s[0]["def"] - 0.5) < 0.01:
                    passed += 1
                else:
                    print(f"  x retune default: {s}")
                    failed += 1
            else:
                failed += 1
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0.95) < 0.01:
                passed += 1
            else:
                print(f"  x set retune: {r7}")
                failed += 1
            ...
            if r8.get("newVal") is not None and abs(r8["newVal"] - 7) < 0.01:
                passed += 1
            else:
                print(f"  x set key: {r8}")
                failed += 1

            await bridge.stop()
            print(f"werkstatt_auto_tune E2E: {passed}/{passed + failed}")
