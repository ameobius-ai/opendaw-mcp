#!/usr/bin/env python3
"""E2E test for werkstatt_bass_enhancer.js — psychoacoustic bass enhancer."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_bass_enhancer.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            # Test 3: compile bass_enhancer code
            ...
            expected_labels = {"freq", "sub_level", "direct_level", "harmonics",
                               "attack", "release", "mix", "output"}
            if r3.get("params"):
            ...
            # Test 5: freq default = 80
            if r3.get("params"):
                f = [p for p in r3["params"] if p["label"] == "freq"]
                if f and abs(f[0]["def"] - 80) < 1:
                    passed += 1
                else:
                    print(f"  ❌ freq default: {f}")
                    failed += 1
            else:
                failed += 1
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 120) < 1:
                passed += 1
            else:
                print(f"  ❌ set freq: {r6}")
                failed += 1

            # Test 7: set sub_level = 0.8
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "sub_level") {
                            oldVal = pb.value.getValue();
            ...
            await bridge.stop()
            print(f"werkstatt_bass_enhancer E2E: {passed}/{passed + failed}")
