#!/usr/bin/env python3
"""E2E test for werkstatt_graphic_eq.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            # 3. Compile graphic EQ — 11 params (10 bands + master)
            ...
            if r3.get("compiled") and r3.get("paramCount") == 11:
                labels = [p["label"] for p in r3["params"]]
                print(f"  ✅ test 3: compiled, 11 params: {labels}")
                passed += 1
            else:
                print(f"  ❌ test 3: {r3}")
                failed += 1

            # 4. Set band_1k to +6dB
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 6) < 0.01:
                print(f"  ✅ test 4: band_1k {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1

            # 5. Set band_32 to -8dB
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - (-8)) < 0.01:
                print(f"  ✅ test 5: band_32 {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1

            # 6. Set band_16k to +4dB
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 4) < 0.01:
                print(f"  ✅ test 6: band_16k {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - (-3)) < 0.01:
                print(f"  ✅ test 7: master {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_graphic_eq E2E: {passed}/{passed+failed}")
            return failed == 0
