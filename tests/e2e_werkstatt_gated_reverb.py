#!/usr/bin/env python3
"""E2E test for werkstatt_gated_reverb.js — 80s gated reverb."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_gated_reverb.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            # Test 3: compile gated_reverb code
            ...
            expected_labels = {"decay", "predelay", "damping", "width", "threshold",
                               "hold", "release", "mix", "output"}
            if r3.get("params"):
            ...
            # Test 5: decay default = 0.5
            if r3.get("params"):
                d = [p for p in r3["params"] if p["label"] == "decay"]
                if d and abs(d[0]["def"] - 0.5) < 0.01:
                    passed += 1
                else:
                    print(f"  ❌ decay default: {d}")
                    failed += 1
            else:
                failed += 1

            # Test 6: set decay = 0.8 (long reverb)
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 0.8) < 0.01:
                ...
                print(f"  ❌ set decay: {r6}")
                failed += 1

            # Test 7: set threshold = 0.1 (higher gate threshold)
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0.1) < 0.01:
                passed += 1
            else:
                print(f"  ❌ set threshold: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"werkstatt_gated_reverb E2E: {passed}/{passed + failed}")
