#!/usr/bin/env python3
"""E2E test for werkstatt_formant_filter.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            # 3. Compile formant filter — 9 params
            ...
            if r3.get("compiled") and r3.get("paramCount") == 9:
                ...
                print(f"  ✅ test 3: compiled, 9 params: {labels}")
                passed += 1
            else:
                print(f"  ❌ test 3: {r3}")
                failed += 1

            # 4. Set vowel to 2 (/i/ heed)
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 2) < 0.01:
                print(f"  ✅ test 4: vowel {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1

            # 5. Set formant_a to 500 Hz
            r5 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "formant_a") {
                            oldVal = pb.value.getValue();
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - 500) < 0.01:
                print(f"  ✅ test 5: formant_a {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1

            # 6. Set resonance to 0.95
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 0.95) < 0.01:
                print(f"  ✅ test 6: resonance {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1

            # 7. Set bandwidth_a to 0.12
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "bandwidth_a") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(0.12);
                            newVal = pb.value.getValue();
                        }
                    }
                });
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0.12) < 0.01:
                print(f"  ✅ test 7: bandwidth_a {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_formant_filter E2E: {passed}/{passed+failed}")
            return failed == 0
