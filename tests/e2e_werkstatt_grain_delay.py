#!/usr/bin/env python3
"""E2E test for werkstatt_grain_delay.js — granular delay."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_grain_delay.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            expected_labels = {"delay", "grain_size", "grain_rate", "pitch", "scatter",
                               "pan", "reverse", "feedback", "mix", "output"}
            if r3.get("params"):
            ...
            if r3.get("params"):
                d = [p for p in r3["params"] if p["label"] == "delay"]
                if d and abs(d[0]["def"] - 150) < 1:
                    passed += 1
                else:
                    print(f"  ❌ delay default: {d}")
                    failed += 1
            else:
                failed += 1
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0.8) < 0.01:
                passed += 1
            else:
                print(f"  ❌ set scatter: {r7}")
                failed += 1
            ...
            r8 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "grain_rate") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(40);
                            newVal = pb.value.getValue();
                        }
                    }
                });
            ...
            if r8.get("newVal") is not None and abs(r8["newVal"] - 40) < 0.01:
                passed += 1
            else:
                print(f"  ❌ set grain_rate: {r8}")
                failed += 1

            await bridge.stop()
            print(f"werkstatt_grain_delay E2E: {passed}/{passed + failed}")
