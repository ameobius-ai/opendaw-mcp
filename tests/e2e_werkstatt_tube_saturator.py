#!/usr/bin/env python3
"""E2E test for werkstatt_tube_saturator.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 0.7) < 0.01:
                print(f"  ✅ test 4: drive {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - 0.8) < 0.01:
                print(f"  ✅ test 5: warmth {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - (-0.3)) < 0.01:
                print(f"  ✅ test 6: bias {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1
            ...
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "tone") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(0.2);
                            newVal = pb.value.getValue();
                        }
                    }
                });
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0.2) < 0.01:
                print(f"  ✅ test 7: tone {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_tube_saturator E2E: {passed}/{passed+failed}")
            return failed == 0
