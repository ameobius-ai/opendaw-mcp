#!/usr/bin/env python3
"""E2E test for werkstatt_auto_pan.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            if r3.get("compiled") and r3.get("paramCount") == 6:
                labels = [p["label"] for p in r3["params"]]
                print(f"  ✅ test 3: compiled, 6 params: {labels}")
                passed += 1
            else:
                print(f"  ❌ test 3: {r3}")
                failed += 1
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 5) < 0.01:
                print(f"  ✅ test 4: rate {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - 0.9) < 0.01:
                print(f"  ✅ test 5: depth {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1

            # 6. Set shape to 1 (square)
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 1) < 0.01:
                print(f"  ✅ test 6: shape {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1

            # 7. Set phase to 90 degrees
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "phase") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(90);
                            newVal = pb.value.getValue();
                        }
                    }
                });
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 90) < 0.01:
                print(f"  ✅ test 7: phase {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_auto_pan E2E: {passed}/{passed+failed}")
            return failed == 0
