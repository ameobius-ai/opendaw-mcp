#!/usr/bin/env python3
"""E2E test for werkstatt_comb_filter.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            # 3. Compile comb filter — 5 params
            ...
            r4 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "freq") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(1000);
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 1000) < 0.01:
                print(f"  ✅ test 4: freq {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1

            # 5. Set feedback to -0.5 (negative comb)
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - (-0.5)) < 0.01:
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 0.8) < 0.01:
                print(f"  ✅ test 6: damping {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1

            # 7. Set polarity to 1 (negative comb)
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 1) < 0.01:
                print(f"  ✅ test 7: polarity {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_comb_filter E2E: {passed}/{passed+failed}")
            return failed == 0
