#!/usr/bin/env python3
"""E2E test for werkstatt_multiband_saturator.js — multiband saturation."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_multiband_saturator.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            expected_labels = {"crossover1", "crossover2", "low_drive", "mid_drive", "high_drive",
                               "low_char", "mid_char", "high_char", "output", "mix"}
            if r3.get("params"):
            ...
            if r3.get("params"):
                d = [p for p in r3["params"] if p["label"] == "low_drive"]
                if d and abs(d[0]["def"] - 0.4) < 0.01:
                    passed += 1
                else:
                    print(f"  ❌ low_drive default: {d}")
                    failed += 1
            else:
                failed += 1
            ...
            r6 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "mid_drive") {
            ...
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "high_char") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(0);
                            newVal = pb.value.getValue();
                        }
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0) < 0.01:
                passed += 1
            else:
                print(f"  ❌ set high_char: {r7}")
                failed += 1
            ...
            if r8.get("newVal") is not None and abs(r8["newVal"] - 400) < 1:
                passed += 1
            else:
                print(f"  ❌ set crossover1: {r8}")
                failed += 1

            await bridge.stop()
            print(f"werkstatt_multiband_saturator E2E: {passed}/{passed + failed}")
