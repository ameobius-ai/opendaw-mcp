#!/usr/bin/env python3
"""E2E test for werkstatt_modal_resonator.js — modal synthesis resonator bank."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_modal_resonator.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            expected_labels = {"material", "fundamental", "decay", "brightness", "inharmonicity", "mix", "output"}
            if r3.get("params"):
            ...
            if r3.get("params"):
                f = [p for p in r3["params"] if p["label"] == "fundamental"]
                if f and abs(f[0]["def"] - 220) < 1:
                    passed += 1
                else:
                    print(f"  ❌ fundamental default: {f}")
                    failed += 1
            else:
                failed += 1
            ...
            r8 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "inharmonicity") {
                            oldVal = pb.value.getValue();
            ...
            if r8.get("newVal") is not None and abs(r8["newVal"] - 0.7) < 0.01:
                passed += 1
            else:
                print(f"  ❌ set inharmonicity: {r8}")
                failed += 1

            await bridge.stop()
            print(f"werkstatt_modal_resonator E2E: {passed}/{passed + failed}")
