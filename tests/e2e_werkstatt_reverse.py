#!/usr/bin/env python3
"""E2E test for werkstatt_reverse.js DSP script."""
...
CODE = open(os.path.join(os.path.dirname(SCRIPT_DIR), "scripts", "werkstatt_reverse.js")).read()
        ...
        async def run():
            ...
            r4 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "chunk_size") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(2.0);
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 2.0) < 0.01:
                print(f"  ✅ test 4: chunk_size {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1

            # 5. Set speed to 2.0 (double speed reverse)
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - 2.0) < 0.01:
                print(f"  ✅ test 5: speed {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1

            # 6. Set feedback to 0.5
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 0.5) < 0.01:
                print(f"  ✅ test 6: feedback {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1

            # 7. Set stereo_mode to 1 (ping-pong)
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "stereo_mode") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(1);
                            newVal = pb.value.getValue();
                        }
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 1) < 0.01:
                print(f"  ✅ test 7: stereo_mode {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_reverse E2E: {passed}/{passed+failed}")
            return failed == 0
