#!/usr/bin/env python3
"""E2E test for werkstatt_multiband_comp.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            # 3. Compile multiband comp — 17 params
            ...
            if r3.get("compiled") and r3.get("paramCount") == 18:
                labels = [p["label"] for p in r3["params"]]
                print(f"  ✅ test 3: compiled, 17 params: {labels}")
                passed += 1
            else:
                print(f"  ❌ test 3: {r3}")
                failed += 1

            # 4. Set crossover1 to 500 Hz
            r4 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "crossover1") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(500);
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 500) < 0.01:
                print(f"  ✅ test 4: crossover1 {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1

            # 5. Set low_ratio to 6
            r5 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "low_ratio") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(6);
                            newVal = pb.value.getValue();
                        }
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - 6) < 0.01:
                print(f"  ✅ test 5: low_ratio {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1

            # 6. Set high_gain to +3dB
            r6 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "high_gain") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(3);
                            newVal = pb.value.getValue();
                        }
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 3) < 0.01:
                print(f"  ✅ test 6: high_gain {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1

            # 7. Set mid_threshold to 0.4
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "mid_threshold") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(0.4);
                            newVal = pb.value.getValue();
                        }
                    }
                });
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0.4) < 0.01:
                print(f"  ✅ test 7: mid_threshold {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_multiband_comp E2E: {passed}/{passed+failed}")
            return failed == 0
