#!/usr/bin/env python3
"""E2E test for werkstatt_harmonizer.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            # 4. Set shift1_semi to 12 (octave up)
            r4 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "shift1_semi") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(12);
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 12) < 0.01:
                print(f"  ✅ test 4: shift1_semi {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1

            # 5. Set shift2_semi to -7 (fifth down)
            r5 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "shift2_semi") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(-7);
                            newVal = pb.value.getValue();
                        }
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - (-7)) < 0.01:
                print(f"  ✅ test 5: shift2_semi {r5['oldVal']} → {r5['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 5: {r5}")
                failed += 1
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 0.5) < 0.01:
                print(f"  ✅ test 6: detune {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1

            # 7. Set shift1_cent to 10
            r7 = await bridge.evaluate("""async () => {
                ...
                h.modify(() => {
                    for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                        const pb = pointer.box;
                        if (pb.label.getValue() === "shift1_cent") {
                            oldVal = pb.value.getValue();
                            pb.value.setValue(10);
                            newVal = pb.value.getValue();
                        }
                    }
                });
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 10) < 0.01:
                print(f"  ✅ test 7: shift1_cent {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_harmonizer E2E: {passed}/{passed+failed}")
            return failed == 0
