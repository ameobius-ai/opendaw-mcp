#!/usr/bin/env python3
"""E2E test for werkstatt_tape_delay.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 0.5) < 0.01:
                print(f"  ✅ test 4: time {r4['oldVal']} → {r4['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 4: {r4}")
                failed += 1
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 0.6) < 0.01:
                print(f"  ✅ test 6: wow {r6['oldVal']} → {r6['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 6: {r6}")
                failed += 1
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0.8) < 0.01:
                print(f"  ✅ test 7: flutter {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_tape_delay E2E: {passed}/{passed+failed}")
            return failed == 0
