#!/usr/bin/env python3
"""E2E test for werkstatt_vibrato.js DSP script."""
...
def main():
    ...
    try:
        ...
        async def run():
            ...
            if r3.get("compiled") and r3.get("paramCount") == 4:
                labels = [p["label"] for p in r3["params"]]
                print(f"  ✅ test 3: compiled, 4 params: {labels}")
                passed += 1
            else:
                print(f"  ❌ test 3: {r3}")
                failed += 1
            ...
            if r4.get("newVal") is not None and abs(r4["newVal"] - 8.0) < 0.01:
            ...
            if r5.get("newVal") is not None and abs(r5["newVal"] - 0.008) < 0.01:
            ...
            if r7.get("newVal") is not None and abs(r7["newVal"] - 0.3) < 0.01:
                print(f"  ✅ test 7: stereo {r7['oldVal']} → {r7['newVal']}")
                passed += 1
            else:
                print(f"  ❌ test 7: {r7}")
                failed += 1
            ...
            await bridge.stop()
            print(f"\n{'='*40}")
            print(f"werkstatt_vibrato E2E: {passed}/{passed+failed}")
            return failed == 0
