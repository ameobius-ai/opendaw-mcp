"""E2E test for werkstatt_compressor.js DSP script."""
...
async def main():
    ...
    # 3. Compile compressor code via ScriptCompiler
    ...
    print("3. Compile compressor:", f"compiled={r3.get('compiled')}, error={r3.get('error')}, params={r3.get('paramCount')}")
    if r3.get("error"):
    ...
    expected = ["threshold", "ratio", "attack", "release", "makeup", "mix", "knee"]
    for e in expected:
    ...
    assert abs(r4["newVal"] - 0.75) < 0.01, f"threshold not set: {r4}"
    ...
    print("5. Set ratio:", r5)
    assert abs(r5["newVal"] - 0.85) < 0.01, f"ratio not set: {r5}"
    print(f"   ratio {r5['oldVal']} → {r5['newVal']} ✅")
