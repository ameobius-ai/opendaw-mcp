#!/usr/bin/env python3
"""E2E test for werkstatt_freq_shifter.js — SSB frequency shifter."""
...
CODE = open(os.path.join(REPO_DIR, "scripts", "werkstatt_freq_shifter.js")).read()
...
def run_test():
    ...
    try:
        ...
        async def test():
            ...
            # Test 3: compile freq_shifter code
            ...
            expected_labels = {"shift", "direction", "feedback", "mix", "output"}
            if r3.get("params"):
            ...
            # Test 5: shift default = 200
            if r3.get("params"):
                s = [p for p in r3["params"] if p["label"] == "shift"]
                if s and abs(s[0]["def"] - 200) < 0.5:
                    passed += 1
                else:
                    print(f"  ❌ shift default: {s}")
                    failed += 1
            else:
                failed += 1

            # Test 6: set shift = 500
            ...
            if r6.get("newVal") is not None and abs(r6["newVal"] - 500) < 1:
                passed += 1
            else:
                print(f"  ❌ set shift: {r6}")
                failed += 1

            # Test 7: set feedback = 0.5
            ...
            await bridge.stop()
            print(f"werkstatt_freq_shifter E2E: {passed}/{passed + failed}")
