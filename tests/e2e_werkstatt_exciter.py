"""E2E test for werkstatt_exciter.js DSP script."""
...
CODE = open("scripts/werkstatt_exciter.js").read()
...
async def main():
    ...
    # 3. Compile exciter code via ScriptCompiler
    ...
    print("3. Compile exciter:", f"compiled={r3.get('compiled')}, error={r3.get('error')}, params={r3.get('paramCount')}")
    if r3.get("error"):
    ...
    expected = ["freq", "harmonics", "drive", "mix", "output"]
    for e in expected:
    ...
    print("4. Set freq:", r4)
    assert abs(r4["newVal"] - 0.75) < 0.01, f"freq not set: {r4}"
    print(f"   freq {r4['oldVal']} → {r4['newVal']} ✅")
    
    # 5. Set harmonics param
    r5 = await bridge.evaluate("""async () => {
        ...
        h.modify(() => {
            for (const pointer of werkstatt.parameters.pointerHub.filter()) {
                const pb = pointer.box;
                if (pb.label.getValue() === "harmonics") {
                    oldVal = pb.value.getValue();
    ...
    print("5. Set harmonics:", r5)
    assert abs(r5["newVal"] - 0.85) < 0.01, f"harmonics not set: {r5}"
    print(f"   harmonics {r5['oldVal']} → {r5['newVal']} ✅")
