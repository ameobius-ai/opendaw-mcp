...
## Tools Added (v1.10.0, 255 total → v1.335.0, 508 total)
...
163. `repeat_notes` — repeat existing notes N times (1-16): cumulative transpose (direction up/down), velocity decay, time gap, cross-track destination
164. `subdivide_notes` — subdivide each note into N parts (2-16): pitch patterns (same/scale/octave/chromatic), velocity patterns (decrescendo/crescendo/accent), diminution, fragmentation
165. `merge_consecutive_notes` — merge same-pitch consecutive notes: same_pitch_only, max_gap_beats, 4 velocity modes (first/last/max/avg), cleanup/sustain
166. `rotate_notes` — cyclic note rotation: 3 axes (position/pitch/both), normalize mod n, preserve_pitch_contour, serialism/jazz rotation
167. `randomize_note_durations` — generative duration variation: 5 modes (uniform/increasing/decreasing/bimodal/jitter), variation 0-1, min/max clamps, preserve_total, seeded PRNG
168. `expand_intervals` — melodic interval expansion/compression: factor 0.25-4.0, 3 anchors (first/last/center), scale snapping (9 scales), motivic development
169. `insert_rests` — positional rest insertion: 3 modes (delete/truncate/shorten), tolerance matching, shorten_neighbors, syncopation/space
...
174. `add_passing_tones` — diatonic passing tones between notes: 12 scales, 4 directions (auto/ascending/descending/nearest), max_interval, velocity/duration_fraction, cross_track option, counterpoint smoothing
175. `add_suspension` — suspension-resolution non-chord tone: preparation → suspension → resolution on strong beats, 10 scales, 3 resolution modes (down/up/both), suspension_offset, preparation_beats, cross_track, Bach/jazz
176. `swap_sections` — swap two sections on timeline: exchange positions, song structure experimentation, handles different-length sections
177. `invert_chord_notes` — chord inversion: 1st-6th inversion at beat position, up (bottom notes up) or down (drop voicing), returns pitches + root
178. `humanize_pitch` — micro-detune (cents) intonation humanization: cents_depth 0-50, bias -20..+20, seeded PRNG, complements humanize_notes
179. `reorder_sections` — full song structure rearrangement: JSON array of section boundaries in new order, rearranges note content back-to-back, complements swap_sections
...
209. `create_montuno` — Latin/jazz piano montuno: 2-3/3-2 clave, guajira, charanga patterns, syncopated chord stabs + melodic passages, I-vi-IV-V auto or custom chords
...
225. `create_korean_percussion` — Korean nongak/samul nori: 5 instruments (janggu chwe+kyong/buk/kkwaenggwari/jing), 5 styles (nongak/samul_nori/binari/utdari_pungnyu/yeongnam_folk), weather element symbolism
...
## Tools Added v1.199.0–v1.335.0 (cycles 199–357, 131 new tools)
