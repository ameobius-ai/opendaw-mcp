...
description: "openDAW automation — 543 MCP tools via Playwright. 80+ orchestration tools. Full DAW control. 12 agent skills. 134 DSP scripts (115 Werkstatt + 9 Apparat + 10 Spielwerk). Stem splitter integration (7 SOTA models, GPU local). Preset save/load (.opb). 6139 unit tests. Scriptable params mapping-aware. Suno integration pipeline (download → analyze → remix → render). See references/"
tags: [opendaw, audio, daw, headless, mcp, playwright]
---

# openDAW Automation Meta-Skill

543 MCP tools (mcp_opendaw_* prefix). Full DAW control via Playwright headless Chromium → openDAW Vite dev server. Published at https://github.com/AMEOBIUS-team/opendaw-mcp (Apache-2.0, CI green). MCP Registry: io.github.AMEBIUS/opendaw-mcp. **12 agent skills** (adaptive-mix-mastering, suno-to-opendaw, suno-prompt-engineering, dsp-script-authoring, opendaw-automation, opendaw-track-architecture, opendaw-sound-design, opendaw-effect-routing, opendaw-genres, opendaw-composition-patterns, opendaw-dsp-chains, lyrics-pipeline). 134 DSP scripts (115 Werkstatt + 9 Apparat + 10 Spielwerk). 6139 unit tests. Suno integration pipeline: download_audio → remix_track → render_full.
...
### Project layout
- MCP server: `opendaw-mcp/server.py` (~65600 lines, 543 tools)
