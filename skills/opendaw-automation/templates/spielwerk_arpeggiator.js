    ...
    process(block, events) {
        ...
        for (let oct = 0; oct < octaves; oct++) {
            for (const n of sorted) {
                ...
                if (p <= 127) seq.push({pitch: p, velocity: n.velocity})
            }
        }
        if (seq.length === 0) return out

        // mode: 0=up, 1=down, 2=up-down, 3=random
        let indices = []
        if (mode === 0) {
        ...
        // CRITICAL: track nextStepPos across blocks — block span (~5ppqn) << rate (240ppqn)
