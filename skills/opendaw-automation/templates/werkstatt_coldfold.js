...
class Processor {
    ...
    process(io, block) {
        ...
        const foldIter = Math.floor(this.fold * 4 + 1)
        const crushBits = Math.max(1, Math.floor(16 - this.crush * 15))
        const crushStep = 2 / Math.pow(2, crushBits)
        ...
        for (let i = block.s0; i < block.s1; i++) {
            ...
            // wavefolding — mirror signal at thresholds
            while (xL > 1) xL = 2 - xL
            while (xL < -1) xL = -2 - xL
            while (xR > 1) xR = 2 - xR
            while (xR < -1) xR = -2 - xR
            
            // fold iterations for harmonic richness
