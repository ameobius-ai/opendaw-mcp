#!/usr/bin/env python3
"""
openDAW 7-stem headless render template.
7 tracks: anchor + minus + vocal + vocals_pf (doubled) + drums_ft + bass_ft + other_bs6

Based on F04 (July 1, session 7). Includes asyncio.Event download fix (120s timeout for 95MB WAV).

Prerequisites:
  - Vite dev server: `npx vite --port 5174 --host` (NO `npm run dev` — package.json lacks it)
    Requires: `source ~/.nvm/nvm.sh && nvm use 23`
  - Stems in headless-daw/public/stems/ (7 files: anchor, minus, vocal, vocals_pf, drums_ft, bass_ft, other_bs6)
  - opendaw-mcp venv: source ~/projects/creative-studio/agent-daw/opendaw-mcp/venv/bin/activate

Usage:
  python3 opendaw_7stem_render.py

Customize:
  - stems[] array: 7 stems with levels
  - OUTPUT_VOLUME: master output (-3 dB runs hot with 7 stems, peak ~-1.2; -4 safer)
  - Revamp EQ: highShelf/highBell on output AU for air (structural limit — stems are dark)
  - OUTPATH: output file path

Output:
  32-bit float WAV, 48kHz, stereo. Saved via download with asyncio.Event safety.
"""
import asyncio
...
STEMS = [
    ...
    {'url': '/stems/minus.wav',     'name': 'minus',     'level': -5.0},  # F03: was -3 in F01
    {'url': '/stems/vocal.wav',     'name': 'vocal',     'level': -1.0},  # F04: was -2 in F01-F03
    {'url': '/stems/vocals_pf.wav', 'name': 'vocal_pf',  'level': -5.0},  # PolarFormer, doubled
    {'url': '/stems/drums_ft.wav',  'name': 'drums_ft',  'level': -4.0},
    {'url': '/stems/bass_ft.wav',   'name': 'bass_ft',   'level': -8.0},  # F02: was -5 in F01
    {'url': '/stems/other_bs6.wav', 'name': 'other_bs6', 'level': -6.0},
]
OUTPUT_VOLUME = -3.0       # dB, master output (-3 runs hot with 7 stems; -4 safer)
HIGH_SHELF_FREQ = 12000    # Hz, air boost
HIGH_SHELF_GAIN = 4.0      # dB
HIGH_BELL_FREQ = 16000     # Hz, air boost
HIGH_BELL_GAIN = 2.0       # dB
OUTPATH = '/tmp/serebro_4stem/serebro_7stem_render.wav'
# ═══ END CONFIG ═══
    ...
    async with async_playwright() as p:
        ...
        if ok:
            try:
                ...
                if download_error[0]:
                    print(f'Download error: {download_error[0]}', flush=True)
                else:
