...
[![Tests](https://img.shields.io/badge/Tests-334%20unit%20%2B%2020%20E2E-brightgreen)](tests/)
...
**294 MCP tools for agent-native control of [openDAW](https://github.com/andremichelle/openDAW) — a browser-based digital audio workstation.**
...
|---------|-------------|-------------------|
| Full DAW control (294 tools) | ✅ | ❌ (1-10 tools) |
...
The `skills/` directory contains structured skill files for AI agents (Hermes, Claude, etc.) that describe how to use the 294 MCP tools effectively. Each skill covers a specific domain and includes decision points so the agent can adapt to any genre or workflow.
...
| `opendaw-automation` | API reference | 294 MCP tools full reference, bridge architecture, pitfalls, DSP script library (33 scripts), CodeRabbit review patterns. The base skill — others cross-reference it. |
...
See [`TOOL_CATALOG.md`](TOOL_CATALOG.md) for the complete list of 294 tools with parameters and descriptions.
...
- **`augment_notes` transformation tool (294 MCP tools)** — augmentation/diminution: the fourth classical motivic transformation. Multiplies note durations by a factor (0.25-4.0). Two modes: "scale" (phrase slows down/speeds up) and "stretch" (durations only). Completes the four fundamental transformations: transpose, reverse, invert, augment. Think Beethoven 5th recapitulation
- **E2E verified**: augmentation x2 (5 notes), diminution x0.5 (8 notes), stretch mode, no-op, validation — 8/8 tests passed
- **+10 unit tests** for factor validation + duration math (334 total)
...
- **+10 unit tests** for canon voice generation + transposition + direction + clamping (334 total)
