...
def main():
    ...
    # Sub (<200Hz): psychoacoustic bass + parallel comp + mono
    print("  sub: psychoacoustic bass + parallel comp + mono collapse")
    ...
    # Bass (200-500Hz): mud cut + TUBE sat (NEW v7 — even harmonics on lows)
    print("  bass: mud cut -2dB @ 250 + tube sat (warm bass, NEW v7)")
    bands[1] = bell_eq(bands[1], sr, 250, -2.0, q=1.2)
    bands[1] = tube_saturation(bands[1], drive=0.15, mix=0.3)

    # Lowmid (500-2k): warmth fill + pre-emphasis tape sat (NEW v7)
    print("  lowmid: +3dB @ 700 + pre-emphasis tape sat (NEW v7)")
    ...
    # Air (>8k): boost + exciter + widen
    print("  air: high-shelf +4dB + exciter + bell +3dB @ 12k + widen")
    ...
    # ═══ Step 7: Soft knee limiter with program-dependent release (NEW v7) ═══
    print("\n=== Step 7: Soft knee limiter (prog-dependent release, NEW v7) ===")
    data = soft_knee_limiter_prog(data, ceiling=0.84, knee_db=3.0, sr=sr,
                                  lookahead_ms=3.0, release_fast_ms=50.0, release_slow_ms=150.0)
