...
298 MCP tools for headless openDAW control via Playwright bridge.
...
- `create_harmonic_arrangement` — One-call harmonic quartet: creates all four harmonic layers (chord pads + arpeggiated progression + bass + melody) from a single "Am-F-C-G" progression string. Replaces 4 separate calls with 1. Configurable per-layer: pad_octave, arp_pattern (up/down/updown/random/bass or "" to skip), arp_octave, arp_step, bass_pattern (root/root_fifth/walking/pedal/octave/root_octave), bass_octave, melody_pattern (chord_tones/sustained/syncopated/triadic/stepwise or "" to skip), melody_octave. Velocity auto-scaled per layer (pads=0.9x, bass=1.0x, arp=0.85x, melody=0.75x). Melody auto-routed to track 4 when arp present, track 3 otherwise. Jazz preset: arp_pattern="" + bass_pattern="walking" + melody_pattern="sustained". House preset: arp_pattern="bass" + bass_pattern="pedal" + bass_octave=1 + melody_pattern="".
