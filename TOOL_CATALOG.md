...
557 MCP tools for headless openDAW control via Playwright bridge.
...
- `create_chaconne` — Chaconne: repeating bass + repeating chord progression + developing variation melody. 5 variation styles: baroque (descending stepwise, grace notes accumulate), romantic (wide intervals, rubato), jazz (syncopated, chromatic passing, descending runs), minimalist (phase-shifted cells), contemporary (dissonant clusters, pointillistic). Chord parsing: major/minor/dim/aug/maj7/m7/7/sus4/sus2/m7b5. Bass on track_index, chords on track_index+1, variation on track_index+2. Unlike ground bass (bass only) — chaconne fixes both bass and harmony, variations develop on top.
- `create_soli` — Ensemble unison passage with octave doublings. All voices play same melodic line in rhythmic unison at different octaves. 2-5 voices, adjustable octave spread. 14 scales. Scale degree input (0=root, 2=2nd, -1=7th below). Jazz big band soli (Basie/Ellington), orchestral tutti, rock/metal unison riffs. Unlike fugue (polyphonic imitation) or canon (delayed entry) — soli is simultaneous and homorhythmic. Outer voices full velocity, inner voices slightly reduced.
- `create_rondo` — Rondo form: recurring theme (A) alternating with contrasting episodes (B, C, D). 5 form types: simple (ABA), classical (ABACA), seven_part (ABACABA), pop_rock (ABABCB), jazz (ABAC). Each section has distinct melodic pattern (A=tonic stepwise, B=dominant wider, C=distant contrast) and bass pattern. Adjustable bars_per_section. Mozart/Beethoven classical rondo, pop/rock verse-chorus-bridge, jazz standard form. Melody on track_index, bass on track_index+1.
- `create_call_and_response` — Two phrases in musical dialogue: leader (call) followed by response. 5 response types: echo (exact repeat), transpose (interval shift), variation (rhythmic variation), complementary (contrasting phrase), fill (short approach+target). 14 scales. Adjustable gap, pairs, response interval. Root of blues, gospel, African music, jazz, work songs, hip-hop. Call on track_index, response on track_index+1.
- `create_binary_form` — Binary form: two contrasting sections (A|B) with optional repeats (AABB). 5 modulation types: dominant (B in V key), relative (B in relative minor/major), subdominant (B in IV), parallel (same key, different material), no_modulation. A section: stepwise melody around tonic, I-V-I bass. B section: wider intervals, modulated, returns to tonic. Baroque dance suites (Bach), folk tunes, early jazz. Melody on track_index, bass on track_index+1.
- `create_sonata_form` — Sonata form: exposition (theme 1 in tonic + transition + theme 2 in dominant/relative), development (fragmentation, sequence, modulation through iii/vi/ii/IV, dominant pedal retransition), recapitulation (both themes in tonic). The structural foundation of classical symphonies, sonatas, quartets from Haydn through Mahler. Melody on track_index, bass on track_index+1.
- `create_ternary_form` — Ternary form (ABA): outer A sections with contrasting B middle. 5 B-section contrast types: trio (subdominant, smoother rhythm, minuet & trio), dominant (V key, more active, Beethoven scherzo), relative (relative minor/major, Schubert impromptu), episode (same key, different material, Chopin nocturne), development (fragmentation of A material). Optional A' ornamentation (passing tones, trill-like neighbors). Da capo aria, minuet & trio, Chopin nocturnes, pop/jazz ABA. Melody on track_index, bass on track_index+1.
- `create_hardstyle_arrangement` — Hardstyle: 150 BPM Dutch festival hard dance. 4 tracks: hard kick on every beat + snare 2&4 + closed hats offbeat + open hat, reverse bass (offbeat boom-BM pattern), screechy sawtooth lead (wide intervals, octave jumps, high octave), stab chords on 1&3 (i-VI-III-VII). Headhunterz, Brennan Heart, Wildstylez.
- `create_garage_arrangement` — UK garage: 130 BPM 2-step swing. 4 tracks: 2-step kick (alternating syncopated patterns), snare on 2&4, swung 16th hats (+0.08 swing), rim ghost notes, melodic bassline with octave jumps and syncopation, minor 7th chord stabs on offbeats, vocal-chop lead (short rhythmic phrases with gaps). G minor default. MJ Cole, Disclosure, Burial.
...
- `create_solo` — Genre-specific melodic solo generator. 5 styles: bebop (chromatic approaches, enclosures, chord-tone targeting), blues (pentatonic + blue notes, 12-bar, riff repetition), rock (pentatonic positions, register climaxes), jazz_swing (swung 8ths, guide-tone lines), fusion (wide intervals, rhythmic displacement). 6 scales. Seeded PRNG. Charlie Parker to Holdsworth.
- `create_drum_solo` — Genre-specific drum solo with rudimental vocabulary. 5 styles: rock (double kick, tom fills, crash climax), jazz (swing ride, comping, press rolls), funk (ghost-note 16ths, hi-hat splashes), latin (cascara, mambo bell, timbale fills), marching (paradiddles, flams, drags, open rolls). Rudiment helpers. Seeded PRNG. Bonham to DCI.
- `create_riff` — Genre-specific riff generator. 5 styles: rock (power chords, bluesy bends, syncopated rests), funk (16th syncopation, ghost notes, staccato stabs), metal (galloping rhythm, tritone intervals, palm-mute), blues (shuffle feel, pentatonic bending, call-response), hip_hop (sparse loop, melodic pentatonic). 5 scales. Seeded PRNG. Smoke on the Water to Still D.R.E.
- `create_turnaround` — 2-bar resolution phrase generator. 5 styles: jazz (I-vi-ii-V with chromatic approach tones, guide tones, 7ths), blues (I-IV-IVdim-I walkup, shuffle feel, dominant 7ths), gospel (ii-V-I with chromatic approach, plagal Amen cadence, melisma), rock (bVII-IV-I mixolydian descent, power chords, octave accents), pop (I-V-vi-IV axis progression, diatonic stepwise melody). 4 scales. Seeded PRNG. Ellington to Axis of Awesome.
- `create_lick` — Short melodic vocabulary phrase generator. 5 styles: bebop (enclosures, chromatic passing, 8th-note line), blues (blue notes b3/b5/b7, call-response), funk (16th syncopation, octave jumps, ghost notes), rock (pentatonic, bending, climax note), jazz_minor (diminished arpeggio + chromatic descent over minor ii-V-i). 5 scales. Seeded PRNG. Parker to Coltrane.
- `create_hook` — Melodic earworm phrase generator. 5 styles: pop (stepwise singable, I-V-vi-IV contour), rock (pentatonic, power-note climaxes, power chords), dance (rhythmic ostinato, syncopated stabs, octave jumps), rnb (melismatic, blue-note inflections, chromatic turns), country (diatonic story-melody, 3rd/6th emphasis). 5 scales. Seeded PRNG. Seven Nation Army to Don't Stop Believin'.
- `create_etude` — Technical study piece generator. 5 types: scale (ascending/descending runs), arpeggio (broken chords, inversions, I-IV-V-I), interval (parallel thirds, two voices), rhythm (syncopation, varied patterns), chromatic (chromatic runs, chromatic thirds). 5 scales. Seeded PRNG. Czerny to Chopin.
- `create_cadence` — Harmonic phrase conclusion generator. 5 types: authentic (V-I, strongest closure), plagal (IV-I, Amen cadence), half (I-V, no closure, musical comma), deceptive (V-vi, surprise redirect), phrygian (bII-i, Neapolitan, dark exotic). 3 scales. Seeded PRNG. Bach to Miles Davis.
- `create_intro` — Intro section generator. 5 types: ambient (sustained pad swell, root+fifth), drum (rhythmic build kick→snare→hats), melodic (emerging arpeggio I-vi-IV-V, sparse→dense), minimalist (Reich/Glass layered repetition), cinematic (drone+riser+impact, chromatic ascent). 3 scales, 2-8 bars. Seeded PRNG. Film trailers to ambient openings.
- `create_outro` — Outro section generator. 5 types: fade (gradual thinning, velocity decreases), ritardando (slowing, durations increase, fermata), recap (return to opening, descending swell), pedal (V-I resolution, sustained tonic, octave), cadential (running notes to fermata chord, Beethoven close). 3 scales, 2-8 bars. Seeded PRNG. Pairs with create_intro.
- `create_bridge` — Bridge section generator. 5 types: breakdown (sparse bass + stabs), modulation (key shift up m3), solo (dense melodic over I-IV-V-I), atmospheric (sustained chromatic tones), surprise (staccato bursts at odd positions). 3 scales, 2-8 bars. Seeded PRNG. Completes intro→body→bridge→outro framing.
- `create_interlude` — Interlude section generator. 5 types: instrumental (solo melodic break over sparse bass), atmospheric (sustained I-IV-V-I pad chords), breakdown (single root→add fifth→add stabs, layer build), reprise (callback motif at different octave), contrapuntal (2-3 independent lines, contrary motion). 3 scales, 2-4 bars. Seeded PRNG. Connective tissue between sections.
...
- `produce_full_track` — Ultimate meta-tool: produces a complete track in one call. Chains set_bpm → arrange_full_song → create_drum_pattern → create_bassline → create_melody → create_chord_progression → apply_mix_preset → render_full (optional). One call replaces 15-20 individual tool calls. Agent specifies structure, key, genre, tempo — everything else automatic. 37 genres supported.
- `create_descant` — Descant counter-melody generator. 5 types: soaring (long sustained rising notes, anthemic), weaving (interlocking phrases filling melody gaps), pedal_tone (sustained high 5th/tonic ringing above), call_response (short answering phrases), ornamental (fast decorative runs and grace notes, baroque). 3 scales, octave 5+ (above melody), 2-8 bars. Seeded PRNG. Secondary melody above the main tune — choral, worship, folk, ballads.
- `auto_master` — Adaptive mastering meta-tool. Chains analyze_mix → add_mastering_chain → auto_gain → verification meters in one call. 6 platform targets (Spotify -14, Apple -16, YouTube -14, Tidal, SoundCloud, Club -8), 4 styles (balanced, warm, loud, transparent), true peak ceiling control. LUFS meter automatically placed on output for post-mastering verification. One call replaces 3-5 mastering tool calls.
- `add_genre_effects` — Genre-specific effect chains in one call. 37 genres with character-appropriate effect routing: house (sidechain comp + Dattorro + widener), techno (pumping comp + delay + waveshaper), dnb (aggressive comp + reverb + reese distortion), metal (heavy waveshaper + comp), ambient (long reverb, no compression), lofi (gentle comp + subtle saturation), industrial (waveshaper + heavy comp), kpop (comp + reverb + StereoTool), jpop (comp + reverb + delay + StereoTool), harmonic (reverb + gentle comp), etc. Saves 5-10 individual add_effect + set_effect_parameter calls.
- `produce_and_master` — The single most powerful tool. Chains 10 steps in one call: set_bpm → arrange_full_song → create_drum_pattern → create_bassline → create_melody → create_chord_progression → add_genre_effects → auto_master → render_full → read_meter (verification). One call = a fully produced, mastered, and verified track ready for streaming. Replaces 30-40 individual tool calls. 37 genres, 6 platform targets, 4 mastering styles. Closed-loop: master → meter → verify.
- `create_industrial_arrangement` — Industrial music arrangement (NIN/Ministry/Skinny Puppy style): distorted 4-on-floor, metallic percussion (anvil clangs, ride shimmer), chromatic bass with passing tones, dissonant tritone stabs, dark drone pad. D minor, 135 BPM, 16 bars, 4 tracks.
- `create_breakbeat_arrangement` — Breakbeat/big beat arrangement (Prodigy/Chemical Brothers/Fatboy Slim style): syncopated Amen-style broken beat, rolling bassline with octave jumps, acid riff with call-response, vocal chop stabs on offbeats. E minor, 130 BPM, 16 bars, 4 tracks.
- `create_verse` — Verse section generator: the storytelling section. 5 types: narrative (steady stepwise melody, beats 1&3), sparse (minimal chord tones, space for vocals), driving (continuous 8th notes, forward momentum), conversational (short phrases with speech-like rests), build (gradually increasing density toward chorus). 3 scales, 4-16 bars. Seeded PRNG. Lower energy than chorus — where lyrics lead.
- `create_chorus` — Chorus section generator: the emotional peak, the hook. 5 types: anthemic (big wide sustained notes, octave jumps, stadium-filling, I-V-vi-IV), hooky (short catchy 2-bar phrase repeated, earworm), driving (relentless 16th notes, no letup), soaring (melody climbs higher, peak note near end, crescendo), call_response (antecedent-consequent, question then answer). 3 scales, 4-16 bars. Seeded PRNG. Higher energy than verse — what listeners remember.
- `create_kpop_arrangement` — K-pop arrangement (BTS/Blackpink/Stray Kids style): polished maximalist pop, 4-on-floor with clap+snare layering, driving bassline with octave jumps, catchy wide-interval lead with call-response, bright I-V-vi-IV chord stabs. C major, 128 BPM, 16 bars, 4 tracks.
...
- `create_songo_pattern` — Cuban songo drum-kit pattern: fusion of son, rumba, jazz drumming (Los Van Van, Changuito, 1970s). 4 variations: classic, modern (timba-era), fusion (jazz), songo_funk. 4 drum-kit voices: kick (bombo), snare (rim/open/ghost), hi-hat (8ths), tom (fills). Unlike clave/tumbao/cascara (individual percussion), songo is a complete drum-kit groove. Afro-Cuban / Latin jazz.
- `create_samba_pattern` — Brazilian samba percussion ensemble: 5 independent instruments (surdo, caixa, tamborim, chocalho, repique). Unlike songo (drum kit), samba is a bateria ensemble. 4 styles: batucada (carnival), samba_enredo (parade), pagode (informal), samba_funk (fusion). Each instrument layered independently. Brazilian music tradition.
- `create_djembe_ensemble` — West African djembe/dunun ensemble: 6 instruments (kenkeni, sangban, dundunba, bell, djembe2, djembe1). Cyclical ostinato under improvised lead. 4 traditional rhythms: danza, kuku, djole, doundounba. Unlike samba (parade) or songo (kit), West African drumming is cyclical ostinato. Mali, Guinea, Senegal.
- `create_arabic_percussion` — Arabic/Middle Eastern percussion: darbuka (dum/tek/ka), daf (frame drum), zills (finger cymbals). 6 rhythms: maqsum, baladi, saidi, ayoub, malfouf, chiftetelli. Stroke-to-pitch offset (dum=low, tek=high, ka=higher). Arabic, Turkish, Persian.
- `create_flamenco_compas` — Flamenco compás: 12-beat cyclical rhythm. 6 palos: bulerias, solea, alegrias, seguiriyas, tangos, rumba. 4 instruments: palmas secas (sharp), palmas sordas (muffled), cajón (box drum), golpe (tap). Accents on 3/6/8/10/12 for 12-beat palos. Andalusian / Spanish.
- `create_balkan_meter` — Balkan additive meter: asymmetric time signatures (7/8=2+2+3, 9/8=2+2+2+3, 11/16, 13/8) with unequal beat groupings. 6 meters + reversed variants. 3 variations: classic/modern/wedding. Bulgarian, Macedonian, Greek, Serbian.
...
- `create_reggae_percussion` — Jamaican reggae drum patterns: 6 styles covering the evolution of Jamaican popular music. one_drop (roots reggae, kick+snare on beat 3 "the drop"), rockers (kick 1+3, snare 2+4, Sly & Robbie), steppers (four-on-the-floor kick, dub/roots), ska (fast upbeat emphasis, Skatalites), rocksteady (laid-back, behind-beat hats), dancehall (syncopated, programmed feel). GM percussion pitches (kick 36, snare 38, closed hat 42, open hat 46). Swing parameter for ska. Jamaica.
- `create_konokol` — Indian Carnatic konokol (solkattu): vocal percussion as MIDI. 6 tala structures: adi_tala (8-beat, most common), roopaka_tala (6-beat 3/4), khanda_chapu (5-beat asymmetric), mishra_chapu (7-beat lyrical), triputa_tala (7-beat variant), jhampa_tala (10-beat). Syllable-to-pitch mapping (ta/ka→hat, dhi/nam→snare, mi→tom, thom/ghu→bass). Subdivision emphasis (first syllable of each group = full velocity). 14th world rhythm tradition. India.
- `create_comparsa` — Cuban comparsa: carnival procession percussion. 7 instruments (conga low/high/open, claves, cowbell, maracas, guiro). 5 styles: habanera (classic 3-2 clave), santiago (2-3 rumba-influenced, guiro), matanzas (rumba columbia, sparse, no claves), conga_line (marching, bass every beat), comparsa_moderna (16th maracas, salsa-influenced). Ancestor of salsa. Cuba.
...
- `create_country_arrangement` — Twentieth multi-track arrangement. Classic country/Americana across 4 tracks: straight 8th backbeat drums, root-five bass (root beat 1, fifth beat 3), boom-chick guitar (alternating bass note + chord strum — Carter Family/Johnny Cash), major pentatonic fiddle lead (root/2/3/5/6 + blue notes b3/b7). 8-bar form: I-I-IV-I-V-I-IV-I. G default, 120 BPM (two-step). Triads, not 7ths — cleaner than blues.
- `create_metal_arrangement` — Twenty-first multi-track arrangement. Heavy metal — riff-based: double kick drums (16th notes), root-following bass, palm-muted power chord chugging (phrygian dominant), shred lead (minor pentatonic + natural minor). E root, 160 BPM (thrash). Power chords (root+fifth), not triads. Distortion in genre_mix.
- `create_gospel_arrangement` — Twenty-second multi-track arrangement. Gospel — the foundation of soul/R&B: gospel shuffle drums (kick on 1+3, snare on 2+4 with ghost notes, triplet shuffle hats), walking bass (root→3rd→5th→7th approach), Hammond B3 organ (chord stabs on 1+3, sustained on 2+4, Leslie rotation feel), SATB choir (sustained with dynamic swell). I-IV-V-I progression. Ab major default (flat keys traditional for gospel singers). 70-85 BPM.
- `create_edm_arrangement` — Twenty-third multi-track arrangement. Festival/mainstage EDM: 4-on-the-floor kick + claps on 2+4 + 16th closed hats + open hats on offbeats, offbeat sub bass, supersaw chord stabs on 1+3 (wall of sound), arpeggiated lead hook (root→3rd→5th→3rd→oct→5th→3rd→root). Snare roll buildup crescendo at end of phrases. i-VI-III-VII progression (Fm-Db-Ab-Eb = vi-IV-I-V equivalent). F minor default (most common EDM key). 128 BPM.
- `create_harmony_line` — Diatonic harmony from existing melody. 5 intervals (third/sixth/fifth/fourth/octave), direction above/below, diatonic scale steps. Auto-creates target. Use for vocal harmonies, string pads, guitar harmonies."
...
- `create_chord_pads` — Create chord pads from a human-readable progression string (e.g. "Am-F-C-G"). Unlike create_chord_progression (JSON arrays), this takes simple hyphen-separated chord names. 10 chord types: maj, min, dom7, maj7, min7, sus2, sus4, add9, dim, aug. Configurable octave, velocity, bars per chord, note duration. Default: i-VI-III-VII in A minor (synthwave/trance). Also: "C-G-Am-F" = pop I-Vi-IV-V, "Dm7-G7-Cmaj7-Am7" = jazz ii-V-I-vi.
- `create_voice_led_progression` — Create chord pads with smooth voice leading. Same input as create_chord_pads but re-voices each chord for minimal voice movement. Common tones stay stationary, other voices resolve by nearest step. Reports per-chord movement stats. voice_range constrains spread (default ±1 octave). Use for strings/pads that glide instead of jump.
- `reharmonize_progression` — Reharmonize a chord progression with 5 substitution techniques: tritone_sub (V7→♭II7, shared guide tones), secondary_dominant (insert V7 of target), diatonic_sub (I→vi, shared tones), modal_interchange (borrow from parallel key), passing_dim (diminished passing chord). Intensity control + target_chord for selective substitution. Returns reharmonized progression + explanations.
- `invert_chord_notes` — Invert a chord at a specific beat position — voicing change. 1st inversion (3rd in bass), 2nd inversion (5th in bass), up to 6th. Direction "up" (move bottom notes up octave) or "down" (drop voicing — top notes down). Finds notes at beat, groups as chord, moves N notes. Returns original + new pitches, root name.
- `displace_rhythm` — Rhythmic displacement: shift all notes by a fixed offset (positive=laid-back, negative=pushed). Two modes: "shift" (add offset, auto-extends region) and "circular" (rotate within region — new patterns from same notes). Default 1/16 note. Range -4 to 4 beats.
- `thin_notes` — Reduce note density: 3 strategies (interval=keep every Nth, velocity_threshold=remove quiet ghost notes, random=probabilistic removal). preserve_strong_beats keeps beat 1 and 3. Use after AI transcription or dense generation.
- `strum_notes` — Guitar-style strumming: converts simultaneous chord notes into time-offset strums. 3 directions (down/up/random), configurable speed (1/32 to 1/2 note between strings), jitter for humanization. Use after create_chord_pads to make guitar parts.
- `constrain_note_range` — Limit notes to a pitch range: clamp (hard limit) or octave_wrap (shift by ±12 to fit, preserves pitch class). Use for instrument range constraints (guitar, violin, vocal, flute). Falls back to clamp when range < 12 semitones.
- `set_articulation` — Articulation control: legato (notes extend to next, minus micro_gap), staccato (shorten to ratio of available time), tenuto (full duration, no gap). Groups chords by position. Use for string/wind/lead articulation.
- `generate_melody` — Generative melody from scale + contour: 6 contour shapes (ascending/descending/arch/v_shape/wave/random), 5 rhythm patterns (quarter/eighth/syncopated/mixed/sparse), weighted random pitch selection, rest probability. No chord progression needed.
...
- `map_velocity_by_pitch` — Map velocity based on pitch — expressive dynamics from note height. 4 modes: higher_quieter (piano/orchestral natural), lower_quieter (lead synth highs cut through), bell_curve (vocal range emphasis), inverse_bell (experimental). Intensity 0-1 controls strength. pitch_ref sets neutral point. Returns per-octave velocity stats. Use for naturalising flat MIDI velocities.
- `quantize_velocities` — Snap velocities to discrete stepped levels — MPC 16-level mode, uniform velocity, stepped dynamics. 2-128 levels (2=on/off, 4=pp/p/mf/f, 8=classical, 16=MPC, 32=fine). 4 modes: snap (nearest), floor (round down), ceil (round up), round_random (probabilistic). min/max velocity range. Returns level distribution histogram. Use for robotic techno feel, restoring clean velocity tiers, or creating stepped dynamics from humanized data.
- `balance_track_velocities` — Balance velocities across multiple tracks — MIDI mix leveling. 5 presets: mix_balanced (equal), drums_forward (hip-hop/rock), vocal_forward (pop/ballad), pads_quiet (ambient/cinematic), bass_heavy (reggae/trap). Custom mode with target_velocities. Preserves relative dynamics within each track (multiply mode). Returns per-track stats before/after.
- `create_midi_echo` — Create MIDI echo — repeat notes with decaying velocity and optional pitch shift. 4 feedback modes: linear (geometric decay), exponential (faster decay), constant (stutter feel), reverse (build-up). pitch_shift per repeat creates cascading echoes. dest_track for separate echo track. 1-8 repeats, configurable delay. Use for guitar delay throws, synth echo fills, creative repetitions.
- `create_ratchet` — Create ratchet — repeated notes with changing subdivision rate (accelerando/decelerando). 4 modes: accelerate (slow→fast), decelerate (fast→slow), constant (roll), exponential. max_subdivisions (2-64 notes/beat), velocity_decay, pitch_drift (semitones per note). Bach cadences, electronic build-ups, drum fills, baroque ornamentation.
- `repeat_notes` — Repeat existing notes in a region N times with per-repeat transformations. Takes notes already in the region and copies them 1-16 times, each cycle offset in pitch (cumulative transpose, direction up/down), velocity (decay multiplier), and time (gap between cycles). Unlike create_midi_echo (feedback decay) — preserves structure, applies uniform transform. Ideal for sequences, ostinato patterns, motivic development.
...
- `expand_intervals` — Expand or compress melodic intervals by a factor (0.25-4.0). 3 anchors: first (expand forward), last (expand backward), center (symmetric). Optional scale snapping (9 scales). factor=2 doubles intervals (seconds→thirds), factor=0.5 halves (thirds→seconds). Motivic development, melodic transformation.
- `insert_rests` — Insert rests at specified beat positions. 3 modes: delete (remove notes at positions), truncate (cut overlapping notes at rest point), shorten (halve duration). Tolerance matching, shorten_neighbors for cleaner separation. Positional space creation, syncopation, breathing room.
- `shuffle_notes` — Random permutation of notes within a region. 4 modes: pitches (keep rhythm, shuffle melody), rhythm (keep pitches, shuffle timing), full (shuffle all attributes), within_groups (shuffle within beat groups). Seeded mulberry32 PRNG for reproducibility. shuffle_amount 0-1 controls fraction of notes shuffled. preserve_first/last anchors. Generative melodic/rhythmic variation.
- `merge_note_tracks` — Merge notes from a source track into a destination track. 6 overlap resolution strategies: keep_higher_velocity, keep_lower_velocity, keep_source, keep_dest, keep_both, shorten_earlier. Optional transpose, delete_source flag. Consolidate doubled melodies, flatten multi-track MIDI, combine counterpoint into harmony.
- `apply_contour` — Apply a melodic contour shape to existing notes. 6 contours: ascending, descending, arch, inverted_arch, wave, escalating. Redistributes pitches to follow the contour while keeping timing. range_semitones controls span, optional scale snapping (12 scales), preserve_first/last anchors. Complement to analyze_melody (which extracts contour).
- `explode_chords` — Explode chords into separate voice tracks. Detects chord groups by position tolerance, assigns notes to voices by pitch. 3 directions: down (bass first), up (top first), outward (middle to outer). 4 velocity balance modes: natural, equal, top_heavy, fade. 2-8 voices. Target AU indices or source track. Fundamental orchestration: chord progression → individual instrumental parts.
...
- `add_neighbor_tones` — Add upper/lower neighbor tones to embellish existing notes. Splits note into first_part → neighbor (one scale step away) → return. 12 scales, 3 directions (upper/lower/alternating). neighbor_fraction controls duration, neighbor_offset controls position within note. min_duration_beats filters short notes. Optional cross_track. Bach ornaments, jazz ballad fills, country chicken pickin'.
- `add_anticipation` — Add anticipation notes before strong-beat notes. Anticipation arrives early on weak beat, creating forward rhythmic motion. 12 scales, 4 directions (auto/upper/lower/approach). anticipation_offset controls how early, anticipation_fraction controls duration. min_duration_beats filters. Optional cross_track. Jazz syncopation, pop vocal anticipations, salsa montuno, funk guitar stabs. Fourth and final non-chord tone technique (passing ✅, suspension ✅, neighbor ✅, anticipation ✅).
- `repeat_phrase` — Repeat a melodic phrase N times with transposition. Copies all notes from source region, places each repetition after the previous, transposed by fixed interval. Diatonic (scale-preserving) or chromatic transposition. 5 velocity patterns (constant/crescendo/decrescendo/fade_out/build). Optional time_stretch per repetition (accelerating/decelerating). Optional cross_track. Bach fugues, jazz ii-V-I chains, pop chorus lifts, film score ostinato builds.
...
- `create_melodic_polyrhythm` — Melodic polyrhythm: N notes evenly spaced across M beats. 3:4=triplet, 5:4=quintuplet, 7:4=septuplet. Notes ascend/descend through scale (12 scales, up/down/alternate) or use custom pitches. 4 velocity patterns (constant/accent/fade/wave). 1-8 bars. Jazz cross-rhythm, prog-rock metric modulation, African cross-pulse, contemporary classical.
- `create_phase_shift` — Phase-shifted copy with gradual drift — Steve Reich phasing. Copies source phrase, each bar shifts by cumulative offset. forward/backward direction. shift_per_bar (1/32 to 1/2 note), 2-16 bars. velocity_scale for layer distinction. Optional cross_track. Minimalism (It's Gonna Rain, Piano Phase), techno loop phasing, ambient drift, IDM polyrhythmic evolution.
- `create_metric_modulation` — Metric modulation with note-value equivalence. Tempo change that preserves a specific note duration: new_bpm = old_bpm × (new_note_value / old_note_value). 12 note values (whole through thirty_second), direct ratio mode "N:M", optional time signature change. Elliott Carter, Copland, Adams, Dream Theater, Tool. Unlike add_tempo_change (arbitrary BPM), preserves rhythmic continuity.
- `create_additive_rhythm` — Unequal groupings within a bar. Plus-separated group sizes ("3+2+2", "5+3", "2+1+2") create irregular accent patterns. 4 note values, 5 pitch modes (root/scale_up/scale_down/alternating/octave_bounce), group_start/group_end accent placement, velocity decay within groups. Messiaen, Stravinsky, Bartók, math rock, prog metal.
- `shift_mode` — Modal transformation: shift notes from one scale/mode to another, preserving the tonic. Finds which scale degrees differ between from_scale and to_scale, shifts ONLY those notes by the interval difference. minor→dorian = +1 on degree 6, minor→phrygian = -1 on degree 2, major→mixolydian = -1 on degree 7. Unlike force_scale_notes (snaps to nearest), preserves melodic contour — only changed degrees move. 14 scales supported.
...
- `create_montuno` — Latin/jazz piano montuno ostinato. 2-bar or 4-bar repeating figure with syncopated chord stabs and melodic passages. 4 patterns: 2-3 clave, 3-2 clave, guajira (dotted), charanga (flowing). 3 rhythms (8th/16th/quarter). Custom chord progression or auto I-vi-IV-V. Accent beats. Unlike arpeggiator (mechanical cycling), combines harmonic movement + syncopated rhythm + call-and-response.
- `create_voice_exchange` — Imitative counterpoint: pass motifs between voices. 6 transformation modes: imitation (transpose), inversion (mirror intervals), retrograde (reverse time), retrograde-inversion (both), augmentation (stretch), diminution (compress). Optional swap mode (true voice crossing). Time offset for response entry. Unlike clone_track (exact copy), transforms material as it passes between voices — foundation of fugue, canon, Renaissance polyphony.
- `create_bariolage` — Baroque string crossing technique: rapid alternation between fixed pedal pitch and moving notes. 5 moving patterns (scale_asc, scale_desc, scale_wave, arpeggio, chromatic). 3 subdivisions (8th/16th/32nd). Creates two-voice illusion from single voice — pedal as drone anchor, moving notes as melodic interest. Bach, Vivaldi, Handel bariolage passages. Unlike arpeggiator (cycles chord tones), creates layered cross-register texture.
- `create_tuplet_group` — Irrational rhythm subdivision: N notes in a time span normally occupied by M notes. Triplets (3:2), quintuplets (5:4), septuplets (7:4), up to 16 notes. 5 pitch modes (scale_asc/desc, chord, repeated, alternating). Rest positions. Accent on first note. Repeats. Unlike polyrhythm (multiple voices), tuplets subdivide a single voice — squeezed/stretched feel. Chopin, Ligeti, Ferneyhough.
- `create_cadenza` — Unmeasured virtuosic solo passage with rubato. Irregular rhythm, accelerando, fermatas, dramatic pauses. 6 segment types: flourish (runs), leap (wide jumps), trill (oscillation), fermata (held note + pause), cascade (descending arpeggio), climb (ascending crescendo). 4 styles: classical, romantic, jazz, modern. Virtuosic mode. Breath marks. Unlike all other tools (quantized grid), cadenzas use speech-like unmeasured rhythm. Mozart, Beethoven, Liszt cadenzas.
...
- `create_tala` — Indian classical tala: cyclic rhythmic structure with vibhag sections (unequal-length groupings), tali (clap)/khali (wave) markings, theka (tabla bols). 6 talas: teental (16), ektal (12), jhaptal (10), rupak (7), dadra (6), kehartwa (8). 3 laya tempos: vilambit/madhya/drut. Bol-to-pitch mapping (bayan lower, dayan higher). Tali emphasized, khali soft. Indian classical music.
...
- `identify_chords` — Identify chords from existing notes in a region. Groups notes by temporal overlap, matches pitch-class sets against chord templates (maj, min, 7, maj7, min7, sus2, sus4, add9, dim, aug), returns position + root + type + confidence. Use for reverse-engineering harmony, verifying progressions."
- `analyze_harmonic_rhythm` — Analyse how fast chords change and where. Identifies chords (same as identify_chords), then computes chord timeline (position + duration), harmonic rhythm rate (fast/medium/slow), chords per bar, stable sections (4+ bars same chord), active sections (sub-beat changes), modulation detection (6+ distinct roots). Complements identify_chords by focusing on temporal pattern. Use with analyze_song_structure for full form picture. Groups notes by temporal overlap, matches pitch-class sets against 10 chord types (maj/min/dom7/maj7/min7/sus2/sus4/add9/dim/aug). Returns chord name, root, type, time position, alternate names, and note names. Subset matching for chords with extensions. Use after import/transcription to understand harmony.
...
- `detect_scale_from_notes` — Detect musical scale/key from MIDI notes in a region. 15 scales (major, natural/harmonic/melodic minor, dorian, phrygian, lydian, mixolydian, locrian, pentatonic major/minor, blues, hungarian minor, double harmonic, whole tone). Pearson correlation against pitch class histogram, 180 root×scale combinations. Returns best match + 5 alternatives + confidence. Use before force_scale_notes, diatonic_transpose, generate_melody.
...
- `apply_rhythm_pattern` — Apply a rhythmic pattern to existing notes — the inverse of extract_rhythm. Takes a rhythm string ("x.x.x..x") or onset grid ("1,0,1,0,1,0,0,1" with optional velocity values), cycles it to fill the region, and repositions notes onto onset positions. 3 velocity modes (preserve/accent/flat/pattern), 3 duration modes (preserve/staccato/legato). Closes the rhythm analysis→modification loop.
