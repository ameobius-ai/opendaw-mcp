...
- `produce_and_master` — The single most powerful tool. Chains 8 steps in one call: set_bpm → arrange_full_song → create_drum_pattern → create_bassline → create_melody → add_genre_effects → auto_master → render_full. One call = a fully produced and mastered track ready for streaming. Replaces 30-40 individual tool calls. 37 genres, 6 platform targets, 4 mastering styles.
