...
- `produce_full_track` — Ultimate meta-tool: produces a complete track in one call. Chains set_bpm → arrange_full_song → create_drum_pattern → create_bassline → create_melody → apply_mix_preset → render_full (optional). One call replaces 15-20 individual tool calls. Agent specifies structure, key, genre, tempo — everything else automatic. 37 genres supported.
