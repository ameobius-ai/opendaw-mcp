#!/usr/bin/env python3
"""Post-production mastering — PRO3 "EXPENSIVE" version.

Target: -10 LUFS, loud/mastered sound, wide stereo, analog glue.
Upgrades over pro2:
1. LUFS target -10 (was -14) — commercial loudness
2. Soft clipper (tanh waveshaper) before limiter — extra loudness, analog feel
3. Aggressive glue comp (ratio 2.0, mix 0.5) — SSL-style bus punch
4. Wider stereo (0.4 M/S, 0.6 air) — expensive widening
5. Parallel presence comp pushed harder (ratio 4)
6. True peak ceiling -1.0 dBTP (was -1.3)
7. Harmonic exciter + tape sat slightly hotter for analog character
...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        output_name = f"{base}_pro3"

    TARGET_LUFS = -10.0   # EXPENSIVE: commercial loudness
    CEILING = 0.891       # -1.0 dBFS ceiling (true peak ~-0.9 dBTP)
    ...
    # Air (>8k): boost + EVEN harmonic exciter + WIDER (EXPENSIVE: 0.6)
    print("  air: high-shelf +4dB + EVEN exciter + bell +3dB @ 12k + widen 0.6")
    ...
    bands[5] = stereo_widen(bands[5], amount=0.6)

    # ═══ Step 4: Recombine + AGGRESSIVE glue comp (EXPENSIVE) ═══
    print("\n=== Step 4: Recombine bands + glue comp (SSL-style, ratio 2.0) ===")
    data = multiband_recombine(bands)
    data = compressor(data, threshold=0.4, ratio=2.0, attack=0.015, release=0.12, sr=sr, mix=0.5)
    ...
    print("\n=== Step 5: M/S EQ (side HPF 200Hz + side boost +4dB @ 10k + widen 0.4) ===")
    data = ms_eq(data, sr, side_hp_freq=200, side_boost_freq=10000, side_boost_gain=4.0)
