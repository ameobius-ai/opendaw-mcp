#!/usr/bin/env python3
"""Post-production mastering — PRO4 "EXPENSIVE POST-ROCK" version.

Based on pro3, optimized for post-rock / ambient / cinematic:
1. NO sub mono collapse — keeps stereo width from doubled guitars/vocals
2. Target -9 LUFS (was -10) — maximum commercial loudness
3. Ceiling -0.5 dBFS (was -1.0) — squeeze every dB
4. Wider stereo: air 0.7, M/S widen 0.5, side boost +5dB
5. Softer glue (ratio 1.8) — preserves dynamics for post-rock buildups
6. Soft clipper drive 1.5 — more analog saturation
7. Presence comp softer (ratio 3.5) — keeps vocal intimacy

Usage:
  venv/bin/python post_master_pro4.py <input.wav> [output_name]
"""
import sys
import os
import numpy as np
...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        output_name = f"{base}_pro4"

    TARGET_LUFS = -14.0   # pro2 baseline — streaming safe, no clipping
    CEILING = 0.86        # -1.3 dBFS ceiling (true peak ~-1.0 dBTP), same as pro2
    ...
    # Sub (<200Hz): psychoacoustic bass + transient enhancer + parallel comp
    # NO mono collapse — post-rock needs stereo width from doubled guitars/bass
    print("  sub: psychoacoustic bass + transient enhancer + par.comp (NO mono — keeps width)")
    ...
    # Presence (2k-5k): boost + tube sat + PARALLEL comp ratio 4 (EXPENSIVE)
    print("  presence: +5dB @ 3k + +4dB @ 4k + +3dB @ 5k + tube + par.comp (ratio 4)")
    ...
    bands[3] = tube_saturation(bands[3], drive=0.25, mix=0.35)
    ...
    # Air (>8k): boost + EVEN harmonic exciter + WIDER (POST-ROCK: 0.7)
    print("  air: high-shelf +4dB + EVEN exciter + bell +3dB @ 12k + widen 0.7")
    ...
    bands[5] = stereo_widen(bands[5], amount=0.7)

    # ═══ Step 4: Recombine + glue comp (softer for post-rock dynamics) ═══
    print("\n=== Step 4: Recombine bands + glue comp (ratio 1.8, preserves dynamics) ===")
    ...
    print("\n=== Step 5: M/S EQ (side HPF 200Hz + side boost +5dB @ 10k + widen 0.5) ===")
    data = ms_eq(data, sr, side_hp_freq=200, side_boost_freq=10000, side_boost_gain=5.0)
